...
(function () {
  ...
  // Fallback image used when a restaurant image URL is broken or unavailable.
  // This prevents the broken-image icon from appearing on restaurant cards.
  ...
  function initFilterEngine() {
    const searchInput =
      ...
      document.querySelector('.listing-header input') ||
    ...
    document.querySelectorAll('.filter-cuisine-cb').forEach(cb => {
      cb.addEventListener('change', () => {
        selectedCuisines = Array.from(
          document.querySelectorAll('.filter-cuisine-cb:checked')
        ).map(el => el.value);
    ...
    document.querySelectorAll('.sorting-tabs .tab-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        document
          .querySelectorAll('.sorting-tabs .tab-btn')
        ...
        if (sortType.includes('rating')) {
          selectedSort = 'rating';
        } else if (sortType.includes('low to high')) {
          selectedSort = 'price_asc';
        } else if (sortType.includes('high to low')) {
          selectedSort = 'price_desc';
        } else if (sortType.includes('delivery')) {
          selectedSort = 'speed';
        } else {
  ...
  function getFilteredData() {
    ...
    if (
      ...
      let cityMatches = list.filter(
        r =>
          r.city &&
          r.city.toLowerCase() === userLoc.city.toLowerCase()
      );

      if (
        userLoc.locality &&
        userLoc.locality !== 'ALL'
      ) {
        const localityMatches = cityMatches.filter(
          r =>
            r.locality &&
            r.locality.toLowerCase() ===
              userLoc.locality.toLowerCase()
        );
      ...
      list = list.filter(r => {
        ...
        const cuisineMatch =
          ...
          r.cuisine.some(
            c =>
              c &&
              c.toLowerCase().includes(currentSearch)
          );
    ...
    if (selectedPriceMax < Infinity) {
      list = list.filter(
        r => r.priceForTwo <= selectedPriceMax
      );
    }
      ...
      grid.innerHTML = `
        <div style="grid-column: 1 / -1; text-align: center; padding: 4rem 1rem;">
          ...
          <p style="color: var(--text-muted); margin-top: 0.5rem;">
            Try selecting "View All Localities" or clear your filters.
          </p>
          <button
            class="btn btn-primary"
            style="margin-top: 1.5rem;"
            onclick="window.CravioData.setCurrentLocation('ALL', 'ALL'); location.reload();"
          >
            View All Locations
          </button>
        </div>
        ...
        <div class="card-glass restaurant-card">
          <div class="restaurant-img-wrapper">
            <img
              ...
              onerror="this.onerror=null; this.src='${RESTAURANT_FALLBACK_IMAGE}';"
            >
      ...
      allDishes.map(d => `
        ...
        >
          ...
          <div style="flex-grow: 1;">
            <div
              style="font-size: 0.75rem; font-weight: 700; color: ${d.isVeg ? 'var(--accent-green)' : 'var(--primary)'}; margin-bottom: 0.2rem;"
            >
              ${d.isVeg ? '● VEG' : '● NON-VEG'}
            </div>
            ...
            <div
              style="font-size: 1.1rem; font-weight: 800; color: var(--primary); margin-top: 0.25rem;"
            >
              ₹${d.price}
            </div>
          </div>
          ...
          <button
            ...
            onclick="CravioCart.addItem({
              id: '${d.id}',
              productId: ${d.productId},
              name: '${d.name.replace(/'/g, "\\'")}',
              price: ${d.price},
              image: '${d.image}',
              restaurant: '${d.restaurantName.replace(/'/g, "\\'")}',
  ...
  document.addEventListener('DOMContentLoaded', function () {
    if (
      ...
      typeof window.CravioData.ready.then === 'function'
    ) {
      window.CravioData.ready.then(
        initFilterEngine
      );
    } else {
      initFilterEngine();
