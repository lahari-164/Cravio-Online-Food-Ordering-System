...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // This prevents old/sample values from appearing when the modal is opened again.
  function clearAuthFields() {
    ...
    [loginForm, signupForm].forEach(form => {
      ...
      // Reset the form without restoring any old values.
      if (typeof form.reset === 'function') {
        form.reset();
      }

      // Make sure the fields remain empty after reset.
      inputs.forEach(input => {
        input.value = '';
      });
    });
  ...
  function openAuthModal(mode) {
    ...
    // Always clear old/sample login/signup values before opening.
    clearAuthFields();
  ...
  function closeAuthModal() {
    // Clear authentication fields when the modal closes.
    clearAuthFields();
    ...
    if (favBtn) {
      ...
      if (favBtn.classList.contains('active')) {
        ...
        if (window.CravioToast) {
          window.CravioToast('Added to your favorite restaurants!', 'success');
        }
      } else {
        icon.className = 'fa-regular fa-heart';

        if (window.CravioToast) {
          window.CravioToast('Removed from favorites', 'info');
        }
      }
    }
  });
